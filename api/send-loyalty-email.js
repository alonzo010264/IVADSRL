...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const resendResult = await new Promise((resolve, reject) => {
      const resendReq = https.request(options, (resendRes) => {
        let responseBody = '';
        resendRes.on('data', chunk => responseBody += chunk);
        resendRes.on('end', () => {
          try {
            ...
            if (resendRes.statusCode >= 200 && resendRes.statusCode < 300) {
              ...
              reject(new Error(`Resend error status ${resendRes.statusCode}: ${JSON.stringify(parsedData)}`));
            }
          } catch (e) {
